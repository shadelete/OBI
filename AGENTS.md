...
- Модуль `src/updater.js` — перевірка та застосування оновлень через **GitHub Releases**. `checkUpdate({currentVersion})` → `GET api.github.com/repos/shadelete/OBI/releases/latest`, звіряє semver-версії; `applyUpdate({assetUrl, assetName, targetExe, targetJs, targetIcon})` → скачує `OBI-<ver>.zip`, розпаковує (`tar.exe`, fallback `Expand-Archive`), генерує `updater.bat` (wrapper: чекає завершення OBI.exe → замінює `OBI.exe`/`OBI.js`/`icon.bmp` → перезапускає → чистить temp).
- **Dev-версії не оновлюються**: regex `/-(dev|alpha|beta|rc)/i` у версії `app.getVersion()` блокує перевірку. Стабільні релізи — тег `v<ver>` без суфікса. Тому робоча версія `package.json` для розробки пишеться з суфіксом `-dev` (напр. `0.2.0-dev`) і **не йде** в тег/реліз.
